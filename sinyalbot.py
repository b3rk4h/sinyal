...
def monitor_active_signals():
    try:
        for signal in active_signals[:]:
            ...
            # SL check
            if (signal['side'] == 'LONG' and last_price <= signal['sl']) or \
               (signal['side'] == 'SHORT' and last_price >= signal['sl']):
                send_telegram(f"❌ <b>{symbol} - STOP LOSS</b>\nHarga: {last_price:.3f}")
                log_event(f"{symbol} | SL Hit | {last_price:.3f}")
            ...
            if signal['side'] == 'LONG':
                if not signal['notified_tp1'] and last_price >= signal['tp1']:
                    send_telegram(f"✅ <b>{symbol} - TP1</b> @ {last_price:.3f}")
